...
contract Lottery is OwnableUpgradeable {
    ...
    function _buySingleTicket(uint256 _price, uint8[4] memory _numbers) private returns (uint256){
        ...
        for (uint i = 0; i < winningCombinations; i++) {
            userBuyAmountSum[issueIndex][userCombinations[i]] += _price;
        }
        return tokenId;
    }
    ...
    function calculateMatchingRewardAmount() internal view returns (uint256[4] memory) {
        ...
        totalMatched3 += (userBuyAmountSum[issueIndex][combinations[2]]);
        totalMatched3 += (userBuyAmountSum[issueIndex][combinations[3]]);
        totalMatched3 += (userBuyAmountSum[issueIndex][combinations[4]]);
        totalMatched3 -= (totalMatched4 * 4); //Remove overlaps from Matched4 users

        uint256 totalMatched2 = userBuyAmountSum[issueIndex][combinations[5]];
        totalMatched2 += (userBuyAmountSum[issueIndex][combinations[6]]);
        totalMatched2 += (userBuyAmountSum[issueIndex][combinations[7]]);
        totalMatched2 += (userBuyAmountSum[issueIndex][combinations[8]]);
        totalMatched2 += (userBuyAmountSum[issueIndex][combinations[9]]);
        totalMatched2 += (userBuyAmountSum[issueIndex][combinations[10]]);
        totalMatched2 -= (totalMatched3 * 3); //Remove overlaps from Matched3 users
        totalMatched2 -= (totalMatched4 * 6); //Remove overlaps from Matched4 users
    ...
    function getRewardView(uint256 _tokenId) public view returns(uint256) {
        ...
        if (matchingNumber > 1) {
            ...
            reward = (amount * 1e12 * poolAmount) / getMatchingRewardAmount(_issueIndex, matchingNumber);
        }
        return (reward / 1e12);
    }
